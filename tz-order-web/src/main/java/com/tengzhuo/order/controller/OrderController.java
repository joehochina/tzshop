package com.tengzhuo.order.controller;
...
public class OrderController {
	...
	public String createOrder(OrderInfo orderInfo, HttpServletRequest request) {
		...
		//调用服务生成订单
		TZResult tzResult = orderService.createOrder(orderInfo);
		//如果订单生成成功，需要删除购物车
		if (tzResult.getStatus() == 200) {
			//清空购物车
			cartService.clearCartItem(user.getId());
		}
		//把订单号传递给页面
		request.setAttribute("orderId", tzResult.getData());
